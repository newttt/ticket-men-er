<template>
        ...
        <div class="activity-floor">
            <div class="header1">
                <div class="remen">近期热门</div>
                <a class="more" href="">全部</a>
            </div>
            <div class="activities">
                <div class="wrap">
                    <div class="activity-floor-item" v-for="(item,index) in gao" :key="index">
                        <a href="">
                            <div class="poster2">
                                <img :src="item.recommendContent.poster" alt="">
                                <div class="want-watch">
                                    <div class="num">
                                        {{item.recommendContent.venueId}}
                                        <small>万</small>
                                    </div>
                                    <div class="like"></div>
                                </div>
                            </div>
                            <div class="name1">{{item.recommendContent.name}}</div>
                            <div class="price">
                                <span class="price-num">{{item.recommendContent.highPrice}}</span>
...
import {TabBarApi} from "@api";

export default {
    name:"xiao-TabBar",
    async created() {
        let data = await TabBarApi();
        // console.log(data);
        this.TabBarApis = data.data;
        // console.log( this.TabBarApis);
    },
    data(){
        return {
            gao:[]
...
.poster2 img{
    background-image: url(https://img.piaoniu.com/poster/6dfcda603e8aff7e861579ad5b7202123a074412.jpg?imageView2/2/w/220/h/164/format/jpg/q/60);
    width: 2.04rem;
    height: 2.74rem;
}
.want-watch{
